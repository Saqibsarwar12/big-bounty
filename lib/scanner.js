...
async function runScan(target, mode = 'basic', custom = null, opts = {}) {
  ...
  return {
    ...
    findings, phases, summary, toolsUsed: ['fetch', 'doh-cloudflare', 'crt.sh', 'rdap', 'nvd'].concat(isAdv ? ['http-probe', 'dns-brute', 'nuclei-templates', 'tcp-ports', 'js-crawl', 'attack-engine'] : []).concat(process.env.BROWSERBASE_API_KEY && !isBasic ? ['browserbase-remote-browser'] : []),
    note,
  };
}
