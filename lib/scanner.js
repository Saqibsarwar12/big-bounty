...
const { runNuclei } = require('./nuclei-engine');
const { runReconX } = require('./reconx');
const { runRedTeam } = require('./redteam');
const { runAdvancedPack } = require('./advanced');
const { runAiTriage, aiConfigured } = require('./ai');
const { runBurpIntruder, runFfufDiscovery, runKiterunner, runZapFuzzer, runBurpRepeater } = require('./fuzzers');
const { runEndpointDiscoveryAudit } = require('./endpoint-discovery-risks');
...
// ---------------- runner ----------------
async function runScan(target, mode = 'basic', custom = null, opts = {}) {
  ...
  async function track(name, fn) {
    ...
    if (deadline && Date.now() > deadline) {
      const rec = { phase: name, status: 'skipped', error: 'time budget exhausted', ms: 0 };
      phases.push(rec); if (onPhase) try { onPhase(rec); } catch {}
      return [];
    }
    if (onPhase) try { onPhase({ phase: name, status: 'running' }); } catch {}
    try {
      const r = await fn();
      const list = Array.isArray(r) ? r : (r && Array.isArray(r.findings) ? r.findings : []);
      if (Array.isArray(list)) findings.push(...list);
      const rec = { phase: name, status: 'ok', ms: Date.now() - t0, hits: list.length };
      phases.push(rec); if (onPhase) try { onPhase(rec); } catch {}
      return list;
    } catch (e) {
      const rec = { phase: name, status: 'error', error: e.message, ms: Date.now() - t0 };
      phases.push(rec); if (onPhase) try { onPhase(rec); } catch {}
      return [];
    }
  }
  ...
  if (onPhase) try { onPhase({ phase: 'fetch', status: 'running' }); } catch {}
  ...
  try {
    ...
    if (onPhase) try { onPhase({ phase: 'fetch', status: 'ok', ms: Date.now() - started, hits: 0 }); } catch {}
  } catch (e) {
    const fer = { phase: 'fetch', status: 'error', error: e.message };
    if (onPhase) try { onPhase(fer); } catch {}
    return {
      target, finalTarget: url, mode, startedAt: nowIso(), completedAt: nowIso(),
      durationMs: Date.now() - started, httpStatus: 0, redirectChain,
      findings: [{ id: fid('err'), tool: 'fetch', severity: 'high', title: 'Target unreachable — DNS/connect failed. Check the URL (scheme, port, public reachability).', evidence: { error: e.message } }],
      phases: [{ phase: 'fetch', status: 'error', error: e.message }], summary: { critical: 0, high: 1 }, toolsUsed: ['fetch'], note,
    };
  }
  ...
  if (isAdv) {
    ...
    await track('nuclei', () => runNuclei({ url: finalUrl, deadlineMs: 110000 }));
    await track('reconx', () => runReconX({ url: finalUrl, hostname }));
    await track('redteam', () => runRedTeam({ url: finalUrl, hostname, deadline: Date.now() + 90000 }));
    await track('advanced-pack', () => runAdvancedPack({ url: finalUrl, hostname, deadline: Date.now() + 110000 }));
    await track('attacks', async () => {
      const r = await runAttacks({ url: finalUrl, hostname, dirFindings: dirUrls, onPhase: null });
      return (r && r.findings) || [];
    });
    await track('intruder', () => runBurpIntruder({ url: finalUrl, custom, deadline: Date.now() + 60000 }));
    await track('ffuf', () => runFfufDiscovery({ url: finalUrl, deadline: Date.now() + 60000 }));
    await track('kiterunner', () => runKiterunner({ url: finalUrl, deadline: Date.now() + 60000 }));
    await track('zap-fuzzer', () => runZapFuzzer({ url: finalUrl, deadline: Date.now() + 60000 }));
    await track('burp-repeater', () => runBurpRepeater({ url: finalUrl, deadline: Date.now() + 60000 }));
    await track('endpoint-discovery-audit', () => runEndpointDiscoveryAudit({ url: finalUrl, hostname, deadline: Date.now() + 60000 }));
  }
  ...
  // AI analyst — free-tier triage (OpenRouter free -> NVIDIA direct), advanced only
  let aiErr = '';
  if (!isBasic && aiConfigured()) {
    if (onPhase) try { onPhase({ phase: 'ai-triage', status: 'running' }); } catch {}
    const t0 = Date.now();
    try {
      aiVerdict = await runAiTriage({ target: finalUrl, mode, findings, deadlineMs: 150000 });
      phases.push({ phase: 'ai-triage', status: aiVerdict ? 'ok' : 'skipped', ms: Date.now() - t0 });
    } catch (e) {
      aiErr = (e && e.message) || 'ai failed';
      phases.push({ phase: 'ai-triage', status: 'err', error: aiErr, ms: Date.now() - t0 });
    }
    if (onPhase) try { onPhase({ phase: 'ai-triage', status: 'done' }); } catch {}
  }
  ...
  return {
    ...
    findings, phases, summary, ai: aiVerdict, aiError: aiErr || undefined, toolsUsed: ['fetch', 'doh-cloudflare', 'crt.sh', 'rdap', 'nvd'].concat(isAdv ? ['http-probe', 'dns-brute', 'nuclei-templates', 'tcp-ports', 'js-crawl', 'attack-engine', 'redteam', 'advanced-pack', 'burp-intruder', 'ffuf', 'kiterunner', 'zap-fuzzer', 'burp-repeater', 'sourcemap-detector', 'js-reverser', 'osint-archives', 'error-handling-audit', 'header-audit', 'ai-triage'] : []).concat(process.env.BROWSERBASE_API_KEY && !isBasic ? ['browserbase-remote-browser'] : []),
    note,
  };
}
